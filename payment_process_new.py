...
class PromotionEngine:
    ...
    def evaluate_additional_discount(self, context: PromotionContext) -> Optional[PromotionResult]:
        """Evaluate additional discount"""
        # query = """
        ...
        #         )
        #        AND JSON_OVERLAPS(
        #             JSON_EXTRACT(pr.conditions, '$.transaction_types'),
        #             CAST(%s AS JSON)
        #         )
        #         AND JSON_UNQUOTE(JSON_EXTRACT(pr.conditions, '$.currency')) = %s
        ...
        query = """
            ...
            -- check currency exactly
            AND JSON_UNQUOTE(JSON_EXTRACT(pr.conditions, '$.Currency')) = %s

            -- check years
            AND JSON_UNQUOTE(JSON_EXTRACT(pr.conditions, '$.years')) = %s
            ...
            LIMIT 1;
        """
        result = execute_query(query, (context.plan_name, context.transaction_type, context.currency.upper(), context.years))
    ...
    def evaluate_signup_discount(self, context: PromotionContext) -> Optional[PromotionResult]:
        ...
        query = """
            ...
            WHERE pt.code = 'discount'
                ...
                AND JSON_UNQUOTE(JSON_EXTRACT(pr.conditions, '$.Currency')) = 'both'
                AND JSON_EXTRACT(pr.conditions, '$.years') = %s
                AND (pr.valid_from IS NULL OR pr.valid_from <= NOW())
                AND (pr.valid_until IS NULL OR pr.valid_until >= NOW())
            ORDER BY pr.priority DESC
    ...
    def calculate_all_promotions(self, context: PromotionContext) -> Dict[str, Optional[PromotionResult]]:
        ...
        if not has_coupon:
            ...
            elif context.transaction_type == 'additional_discount':
                addition_discount_result = self.evaluate_additional_discount(context)
                if addition_discount_result:
                    results['additional_discount'] = addition_discount_result
...
class StripePaymentProcessor:
    ...
    def create_subscription(self, customer_id: str, plan_name: str, currency: str, years: int, assisted_job_count: int, success_url: str, cancel_url: str, metadata: Dict = None):
        ...
        try:
            ...
            downgrade_discount_type = ''
            downgrade_discount_value = 0

            signup_discount_amount = 0
            upgrade_discount_amount = 0
            downgrade_discount_amount = 0
            ...
            if metadata and 'promotions_applied' in metadata:
                ...
                if promotion_details.get('signup_discount'):
                    signup_discount_type = promotion_details['signup_discount']['discount_type']
                    signup_discount_value = promotion_details['signup_discount']['discount_value']
                    signup_discount_amount = promotion_details['signup_discount']['discount_amount']

                if promotion_details.get('upgrade_discount'):
                    upgrade_discount_type = promotion_details['upgrade_discount']['discount_type']
                    upgrade_discount_value = promotion_details['upgrade_discount']['discount_value']
                    upgrade_discount_amount = promotion_details['upgrade_discount']['discount_amount']

                if promotion_details.get('downgrade_discount'):
                    downgrade_discount_type = promotion_details['downgrade_discount']['discount_type']
                    downgrade_discount_value = promotion_details['downgrade_discount']['discount_value']
                    downgrade_discount_amount = promotion_details['downgrade_discount']['discount_amount']



                # Get additional discount
                # if 'additional_discount' in promotion_details and promotion_details['additional_discount']:
                #     additional_discount_type = promotion_details['additional_discount']['discount_type']
                #     additional_discount_values = promotion_details['additional_discount']['discount_value']

                # additional_discount_value = 0

                # if promotion_details.get('additional_discount'):
                #     additional_discount_type = promotion_details['additional_discount']['discount_type']
                #     additional_discount_value = promotion_details['additional_discount']['discount_value']
                #     additional_discount_amount = promotion_details['additional_discount']['discount_amount']
            ...
            # if signup_discount_type & signup_discount_value & additional_discount_type & additional_discount_value:
            #     allow_promotion_codes = False
            # else:
            #     allow_promotion_codes = True

            # if signup_discount_value > 0 or additional_discount_value > 0 or upgrade_discount_value > 0:
            #     allow_promotion_codes = False
            # else:
            #     allow_promotion_codes = True
            ...
            if signup_discount_value > 0:
                ...
                discounts.append({"coupon": signup_coupon_id})
            ...
            if upgrade_discount_value > 0:
                ...
                discounts.append({"coupon": upgrade_coupon_id})

            downgrade_coupon_id = ''
            if downgrade_discount_value > 0:
                downgrade_coupon_id = self.create_coupon(f"{plan_name} downgrade discount", downgrade_discount_type, downgrade_discount_value, currency, product_id)
                discounts.append({"coupon": downgrade_coupon_id})


            

            # additional_coupon_id = ''
            # if additional_discount_value > 0:
            #     additional_coupon_id = self.create_coupon(f"{plan_name} additional discount", additional_discount_type, additional_discount_value, currency, product_id)
            #     discounts.append({"coupon": additional_coupon_id})
            ...
            #     final_coupon_id = self.create_coupon(f"{plan_name} additional discount", 'flat', final_discount_amount, currency, product_id)
            #     discounts.append({"coupon": final_coupon_id})

            # if additional_discount_value > 0 and additional_discount_value is not None:
            #     checkout_params["discounts"] = [{"coupon": additional_coupon_id}]
    ...
    def create_checkout_session(self, request):
        ...
        try:
            ...
            promotions = promotion_engine.calculate_all_promotions(
                    context=PromotionContext(
                        plan_name=plan_name,
                        transaction_type='additional_discount',
                        original_amount=base_price,
                        currency=currency
                    )
                )
            if promotions and 'additional_discount' in promotions and promotions['additional_discount']:
                    addional = promotions['additional_discount']
                    additional_discount_amount = float(addional.discount_amount)
                    base_price -= additional_discount_amount
                    print(f"[*] Additional discount for plan {plan_name}: -{additional_discount_amount}\n")
            else:
                print(f"[*] No additional discount for plan {plan_name}\n")
            ...
            promotions_to_apply = [
                ...
                ('additional_discount', promotions.get('additional_discount'))
            ]
